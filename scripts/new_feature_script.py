...
def gen_types_block(module: str, cc_name: str, fi: str, feature_type: str) -> str:
    ...
    if feature_type == "base":
        ...
        return (
            ...
            f"from .parameters import {cc_name}Parameters\n\n"
            f"type {fi}Data = Datasource[None, {cc_name}Parameters]\n"
            f"type {fi}Usecase = UsecaseBaseCallData[\n"
            f"    None,\n    None,\n    {cc_name}Parameters,\n]"
        )
...
def gen_usecase_content(project: str, module: str, cc_name: str, fi: str,
    ...
    if feature_type == "base":
        ...
        body = (
            ...
            f"            return ErrorReturn(\n"
            f"                parameters.error(\n"
            f"                    message=\"Erro ao executar {snake}: \" + str(data.result)\n"
            f"                )\n"
            f"            )\n"
            "        else:\n"
            "            return ErrorReturn(\n"
...
def build_facade_imports(module: str, snake: str, cc_name: str, fi: str,
    ...
    lines: list[str] = []
    # py_return_success_or_error (garantir ReturnSuccessOrError) em formato multilinha
    lines.append("from py_return_success_or_error import (")
    lines.append("    ErrorReturn,")
    lines.append("    SuccessReturn,")
    lines.append("    ReturnSuccessOrError,")
    lines.append(")")
...
def build_facade_method(snake: str, cc_name: str, fi: str,
    ...
    m.append(
        ...
        f"{indent*3}# TODO: mapear parâmetros da fachada para os parâmetros do UseCase\n"
        f"{indent*3}error=error,\n{indent*2})"
    )
